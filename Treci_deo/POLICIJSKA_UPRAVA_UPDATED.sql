...
CREATE TABLE PATROLA (
 ...
 VOZILO_ID NUMBER REFERENCES VOZILO(ID),
...
CREATE TABLE ODRZAVA(
 ...
 PRIMARY KEY (TEHNICAR_ID, ALARMNI_SISTEM_ID, POCETNI_DATUM)
 );
...
CHECK (TIP IN ('ULTRAZVUCNI','DETEKCIJE POKRETA', 'DETEKCIJE TOPLOTNOG ODRAZA')) ENABLE;
...
BEGIN
    ...
    ELSE IF :NEW.TIP = 'DETEKCIJE POKRETA'
        THEN
            :NEW.HOR_REZ_IC_KAMERE := NULL;
            :NEW.VER_REZ_IC_KAMERE := NULL;
            :NEW.GORNJA_GRANICA := NULL;
            :NEW.DONJA_GRANICA := NULL;
        ELSE IF :NEW.TIP = 'DETEKCIJE TOPLOTNOG ODRAZA'
            THEN
                :NEW.OSETLJIVOST := NULL;
                :NEW.GORNJA_GRANICA := NULL;
                :NEW.DONJA_GRANICA := NULL;
